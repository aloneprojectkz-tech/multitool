...
# Кэшируем все входящие бизнес-сообщения + автоответ
@router.business_message(F.from_user.id != OWNER_ID)
async def cache_message(message: Message, bot: Bot):
    async with async_session() as session:
        ...
        # Проверяем автоответ
        ai_result = await session.execute(select(AISettings))
        ai_settings = ai_result.scalar_one_or_none()

        await session.commit()

    # Автоответ — если включён и есть текст
    if ai_settings and ai_settings.auto_reply_enabled:
        ...
        if text:
            ...
            try:
                ...
                await bot.send_message(OWNER_ID, f"❌ Ошибка автоответа: {e}")
